...
当前用户已经进入 render 和 video 组。
ROCm 设备可以被系统识别。
Python 中的 PyTorch 可以正常导入，并且 GPU 可用。
English:

After installation, it is recommended to confirm three things:

The current user is in the render and video groups.
ROCm devices are visible to the system.
PyTorch can be imported in Python and the GPU is available.
中文：
